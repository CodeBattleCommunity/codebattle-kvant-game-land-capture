    ...
    public class Bot : Hub
    {
        ...
        public async Task Up(int y_bot, int x_bot)
        {
            if (BlockCoord.IsBlockY(y_bot - 1) == false)
            {
                Y_Bot = y_bot;
                X_Bot = x_bot;
                await Clients.All.SendAsync("Up", Y_Bot--, X_Bot);
            }
            else
            {
                Y_Bot = y_bot;
        ...
        public async Task Down(int y_bot, int x_bot)
        {
            if (BlockCoord.IsBlockY(y_bot + 1) == false)
            {
                Y_Bot = y_bot;
                X_Bot = x_bot;
                await Clients.All.SendAsync("Down", Y_Bot++, X_Bot);
            }
            else
            {
                Y_Bot = y_bot;
        ...
        public async Task Left(int y_bot, int x_bot)
        {
            if (BlockCoord.IsBlockX(x_bot - 1) == false)
            {
                Y_Bot = y_bot;
                X_Bot = x_bot;
                await Clients.All.SendAsync("Left", Y_Bot, X_Bot--);
            }
            else
            {
                Y_Bot = y_bot;
        ...
        public async Task Right(int y_bot, int x_bot)
        {
            if (BlockCoord.IsBlockX(x_bot + 1) == false)
            {
                Y_Bot = y_bot;
                X_Bot = x_bot;
                await Clients.All.SendAsync("Right", Y_Bot, X_Bot++);
            }
            else
            {
                Y_Bot = y_bot;
        ...
        public async Task AddPoint(int y_bot, int x_bot)
        {
            ...
            await Clients.Caller.SendAsync("AddPoint", Y_Point, X_Point);
        }
    }
}
